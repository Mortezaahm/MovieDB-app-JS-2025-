...
// API-nyckel mot TMDB (används här i utbildningssyfte) och var gratis.
...
/**
 *Vi använder en hjälpfunktion som hämtar JSON data från URL.
 *Istället för att behöva skriva fetch varje gång kan vi anropa denna funktion istället när vi behöver hämta JSON
 * Funktionen kastar också automatiskt fel om något går snett
...
async function fetchRandomPopularMovie() {
    try {
        ...
        const data = await fetchJSON(
            tmdb('movie/popular', { page: String(page) })
        )
...
function renderUserMovies(list) {
    ...
    userMoviesList.querySelectorAll('.btn-edit').forEach((btn) =>
        btn.addEventListener('click', async (e) => {
            ...
            const currentTitle =
                btn.closest('.user-movie-item').querySelector('strong')
                    .textContent || ''
